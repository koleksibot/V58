...
//Gambar Url
const bgbot = 'https://i.ibb.co/NTNTj7C/Exq-XDOZUUAQRic-N.jpg'
...
module.exports = async (xinz, msg, smsg, blocked, _afk, welcome) => {
    try {
        ...
        const testqq = (from, desk) => {
            return xinz.sendButtons(from, `Pilihan Anda Salah!!`, `Pilih yang bener, jangan ngasal!!`, `Pilih Enable atau Disable`, [{
                "buttonId": `${prefix}${desk} enable`,
                "buttonText": {
                    "displayText": "ENABLE"
                },
                "type": "RESPONSE"
            }, {
                "buttonId": `${prefix}${desk} disable`,
                "buttonText": {
                    "displayText": "DISABLE"
                },
                "type": "RESPONSE"
        ...
        const mentions = (teks, memberr, id) => {
            let ai = (id == null || id == undefined || id == false) ? xinz.sendMessage(from, teks.trim(), extendedText, {
                contextInfo: {
                    "mentionedJid": memberr
                }
            }) : xinz.sendMessage(from, teks.trim(), extendedText, {
                quoted: msg,
                contextInfo: {
                    "mentionedJid": memberr
                }
        ...
        if (isGroup && isViewOnce && isAntiVO && xinz.mode !== 'self') {
            ...
            let peq = msg.message.viewOnceMessage.message["imageMessage"] ? {
                key: {
                    fromMe: false,
                    participant: sender,
                    id: msg.key.id
            ...
            let pe = await xinz.prepareMessageFromContent(from, msg.message.viewOnceMessage.message, {
                quoted: peq
            })
        ...
        if (isGroup && !isCmd && !isBaileys) {
            ...
            if (_vn.includes(chats.toLowerCase())) {
                xinz.sendMessage(from, fs.readFileSync(`./media/audio/${chats}.mp3`), audio, {
                    quoted: msg,
                    ptt: true
                })
            }
        ...
        // Add hit
        if (isCmd) {
            axios.get('https://api.countapi.xyz/hit/' + setting.botName + '/visits').then(({
                data
            }) => hit.all = data.value)
        ...
        case prefix + 'allmenu': {
            ...
            //xinz.sendButtonsLoc(from, ind.menu(prefix, ucap, pushname, jumlahUser, runtime(process.uptime()), ownerNumber[0].split("@")[0], jumlahHarian, jumlahCommand, sender, time, `*${b.toUpperCase()}*`, levelMenu, xpMenu, reqXp, uangku, role, Limitnya), `Pilih menu dibawah!`, qqppp, await getBuffer(pic), [sender, "0@s.whatsapp.net"]).then((res) => {
                ...
                xinz.sendList(from, ind.menu(prefix, ucap, pushname, jumlahUser, runtime(process.uptime()), ownerNumber[0].split("@")[0], jumlahHarian, jumlahCommand, sender, time, `*${b.toUpperCase()}*`, levelMenu, xpMenu, reqXp, uangku, role, Limitnya), `Pilih menu dibawah!`, `Jangan lupa Donasi ya Kak`, `Pilih Disini`, list, msg, [sender])
            //})
        }
        break
        //------------------< PRAY Command >-------------------        
        case prefix + 'asmaulhusna':
            ...
            fetchJson(`https://rei-api.herokuapp.com/api/islami/asmaulhusna`)
            .then((res) => {
                let list = []
                let startnum = 1
                for (let i = 0; i < res.result.data.length; i++) {
                    let yy = {
                        title: 'No ke-' + startnum++,
                        rows: [{
                            title: `Nama : ${res.result.data[i].latin}\nLatin: ${res.result.data[i].latin}\nArab: ${res.result.data[i].arabic}\nIndonesia: ${res.result.data[i].translation_id}\nEnglish: ${res.result.data[i].translation_en}`,
                            //rowId: `${prefix}asmaulhusna ${res.result.data[i].latin}`
                        }]
                    }
                    list.push(yy)
                }
                xinz.sendList(from, `List Asmaul Husna`, `List Asmaul Husna yang tersedia`, `Diurutkan Berdasarkan Nama Asmaul Husna`, `Pilih Disini`, list, msg)
            })
                .catch((err) => {
            ...
            case prefix + 'niatshalat':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                fetchJson(`https://rei-api.herokuapp.com/api/islami/niatsholat`)
                .then((res) => {
                    let list = []
                    let startnum = 1
                    for (let i = 0; i < res.result.length; i++) {
                        let yy = {
                            title: 'Niat Sholat ke -' + startnum++,
                            rows: [{
                                title: `Niat Sholat : ${res.result[i].name}\nLatin: ${res.result[i].latin}\nArab: ${res.result[i].arabic}\nTerjemahan: ${res.result[i].terjemahan}`,
                                //rowId: `${prefix}asmaulhusna ${res.result.data[i].latin}`
                            }]
                        }
                        list.push(yy)
        ...
        case prefix + 'kisahnabi':
            ...
            fetchJson('https://rei-api.herokuapp.com/api/islami/kisah-nabi?q=' + q)
                .then((res) => {
                    xinz.sendFileFromUrl(from, res.result.image, `Nama: ${res.result.nama}\nUmur: ${res.result.umur}\nLahir: ${res.result.lahir}\nTempat: ${res.result.tempat}\n\n\n${res.result.kisah}`, msg)
                    limitAdd(sender, limit)
                })
                .catch((err) => {
                    xinz.sendMess(ownerNumber[0], `${command} Error:` + err)
        ...
        case prefix + 'wpmobile':
            if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habisa silahkan kirim ${prefix}limit untuk mengecek limit`)
            const pp = await fetchJson(`https://rei-api.herokuapp.com/api/image/` + `${command}`.replace(`${prefix}`, ''))
            const be = await xinz.prepareMessage (from, await getBuffer(pp.result.url), image)
            let qweriio = be.message["ephemeralMessage"] ? be.message.ephemeralMessage : be                
            xinz.sendButtons(from, `${command}`.replace(`${prefix}`, ''), '', `Klik NEXT untuk mengirimkan` + `*${command}*`.replace(`${prefix}`, ''), [{
                'buttonId': `${prefix}${command}`.replace(`${prefix}`, ''),
                'buttonText': {
                    'displayText': 'NEXT'
                },
                'type': 'RESPONSE'
            }], true, null, null, qweriio.message.imageMessage)
            break
        case prefix + 'slap':
            if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            if (mentioned.length !== 0) {
                fetchJson('https://www.nekos.life/api/v2/img/slap').then(({
                    url
                }) => {
                    xinz.sendSticker(from, url, msg, true, [], {
                        contextInfo: {
                            "mentionedJid": [sender, mentioned[0]]
                        }
                    }).then(async (res) => {
                        await sleep(2000);
                        mentions(`@${mentioned[0].split('@')[0]} slapped @${sender.split('@')[0]}`, [sender, mentioned[0]], true)
                    })
                })
            } else {
        ...
        case prefix + 'pat':
            if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            if (mentioned.length !== 0) {
                fetchJson('https://www.nekos.life/api/v2/img/pat').then(({
                    url
                }) => {
                    xinz.sendSticker(from, url, msg, true, [], {
                        contextInfo: {
                        ...
                        await sleep(2000);
                        mentions(`@${sender.split('@')[0]} pat @${mentioned[0].split('@')[0]}`, [sender, mentioned[0]], true)
                    })
                })
            } else {
                reply(`Penggunaan ${command} @tag`)
        ...
        case prefix + 'baka':
            if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            if (mentioned.length !== 0) {
                fetchJson('https://www.nekos.life/api/v2/img/baka').then(({
                    url
                }) => {
                    xinz.sendSticker(from, url, msg, true, [], {
                        contextInfo: {
        ...
        case prefix + 'hug':
            if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            if (mentioned.length !== 0) {
                fetchJson('https://rei-api.herokuapp.com/api/image/hug').then(({
                    result
                }) => {
                    xinz.sendSticker(from, result.url, msg, true, [], {
                        contextInfo: {
                            "mentionedJid": [sender, mentioned[0]]
                        }
                    }).then(async (res) => {
                        await sleep(2000);
                        mentions(`@${mentioned[0].split('@')[0]} hugged @${sender.split('@')[0]}`, [sender, mentioned[0]], true)
                    })
                })
            } else {
                reply(`Penggunaan ${command} @tag`)
        ...
        case prefix + 'kiss':
            if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            if (mentioned.length !== 0) {
                fetchJson('https://www.nekos.life/api/v2/img/kiss').then(({
                    url
                }) => {
                    xinz.sendSticker(from, url, msg, true, [], {
                        contextInfo: {
                        ...
                        await sleep(2000);
                        mentions(`@${mentioned[0].split('@')[0]} kissed @${sender.split('@')[0]}`, [sender, mentioned[0]], true)
                    })
                })
            } else {
                reply(`Penggunaan ${command} @tag`)
        ...
        case prefix + 'wife': {
            if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            fetchJson('https://api.reiyuura.me/api/image/waifu').then((data) => {
                ...
                .then((res) => {
                    ...
                    for (let x of res.data) {
                        const yy = {
                            ...
                            rowId: `${prefix}mangaa ${x.mal_id}`
                        }
                        list.push(yy)
                    }
                    xinz.sendListMsg(from, `List Manga`, `Hasil Pencarian Manga "${q}"`, `Pilih Manga Anda`, `Pilih Disini`, `Pilih Untuk More Info`, list, msg)
                })
                .catch((err) => {
                    xinz.sendMess(ownerNumber[0], `${command} Error:` + err)
                    reply(mess.error.api)
        ...
        case prefix + 'ytdl': {
            ...
            let list = [{
                title: `Mau dikirim Audio Apa Video bang`,
                rows: [{
                    title: `Audio/Mp3`,
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
        ...
        case prefix + 'pinsearch':
        case prefix + 'pinterestsearch': {
            if (!isPremium) return reply(mess.OnlyPrem)
        ...
        case prefix + 'toaudio':
            ...
            if (isVideo || isQuotedVideo) {
                ...
                exec(`ffmpeg -i ${media} ${ran}`, async (err) => {
                    fs.unlinkSync(media)
                    if (err) return reply('Gagal :V')
                    xinz.sendMessage(from, fs.readFileSync(ran), audio, {
                        quoted: msg
                    })
                    limitAdd(sender, limit)
                    fs.unlinkSync(ran)
        ...
        case prefix + 'petpet': {
            ...
            if (isImage || isQuotedImage) {
                ...
                let a = await petpet(link, {
                    resolution: 1080
                })
                fs.unlinkSync(yoooo)
                fs.writeFileSync('petpet.gif', a)
                await WSF.createSticker('petpet.gif', {
                    type: 'full',
                    pack: setting.packSticker,
                    author: setting.authorSticker,
                    categories: ["❤", " 😍", " 😘", " 💕", " 😻", " 💑", " 👩‍❤‍👩", " 👨‍❤‍👨", " 💏", " 👩‍❤‍💋‍👩", " 👨‍❤‍💋‍👨", " 🧡", " 💛", " 💚", " 💙", " 💜", " 🖤", " 💔", " ❣", " 💞", " 💓", " 💗", " 💖", " 💘", " 💝", " 💟", " ♥", " 💌", " 💋", " 👩‍❤️‍💋‍👩", " 👨‍❤️‍💋‍👨", " 👩‍❤️‍👨", " 👩‍❤️‍👩", " 👨‍❤️‍👨", " 👩‍❤️‍💋‍👨", " 👬", " 👭", " 👫", " 🥰", " 😚", " 😙", " 👄", " 🌹", " 😽", " ❣️", " ❤️", "😀", " 😃", " 😄", " 😁", " 😆", " 😅", " 😂", " 🤣", " 🙂", " 😛", " 😝", " 😜", " 🤪", " 🤗", " 😺", " 😸", " 😹", " ☺", " 😌", " 😉", " 🤗", " 😊", "☹", " 😣", " 😖", " 😫", " 😩", " 😢", " 😭", " 😞", " 😔", " 😟", " 😕", " 😤", " 😠", " 😥", " 😰", " 😨", " 😿", " 😾", " 😓", " 🙍‍♂", " 🙍‍♀", " 💔", " 🙁", " 🥺", " 🤕", " ☔️", " ⛈", " 🌩", " 🌧", "😯", " 😦", " 😧", " 😮", " 😲", " 🙀", " 😱", " 🤯", " 😳", " ❗", " ❕", " 🤬", " 😡", " 😠", " 🙄", " 👿", " 😾", " 😤", " 💢", " 👺", " 🗯️", " 😒", " 🥵", "👋", "🎊", " 🎉", " 🎁", " 🎈", " 👯‍♂️", " 👯", " 👯‍♀️", " 💃", " 🕺", " 🔥", " ⭐️", " ✨", " 💫", " 🎇", " 🎆", " 🍻", " 🥂", " 🍾", " 🎂", " 🍰", "🌃"]
                }).then((buffer) => xinz.sendMessage(from, buffer, sticker, {
                    quoted: msg
                }))
            ...
            if (isQuotedMsg) {
                ...
                ptt.save(ranm, quotedMsg.chats, function() {
                        exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
                            ...
                            xinz.sendMessage(from, fs.readFileSync(rano), audio, {
                                quoted: msg,
                                ptt: true
                            })
                            fs.unlinkSync(rano)
        ...
        case prefix + 'nuliskiri': {
            ...
            spawn('convert', [
                ...
                .on('exit', () => {
                    xinz.sendMessage(from, fs.readFileSync('./media/nulis/images/buku/setelahkiri.jpg'), image, {
                        quoted: msg,
        ...
        case prefix + 'nuliskanan': {
            ...
            spawn('convert', [
                ...
                .on('error', () => reply(mess.error.api))
                .on('exit', () => {
                    xinz.sendMessage(from, fs.readFileSync('./media/nulis/images/buku/setelahkanan.jpg'), image, {
                        quoted: msg,
                        caption: `Jangan malas pak...`
                    })
                    limitAdd(sender, limit)
        ...
        case prefix + 'foliokiri': {
            ...
            spawn('convert', [
                ...
                .on('error', () => reply(mess.error.api))
                .on('exit', () => {
                    xinz.sendMessage(from, fs.readFileSync('./media/nulis/images/folio/setelahkiri.jpg'), image, {
                        quoted: msg,
                        caption: `Jangan malas pak...`
                    })
                    limitAdd(sender, limit)
        ...
        case prefix + 'foliokanan': {
            ...
            spawn('convert', [
                ...
                .on('error', () => reply(mess.error.api))
                .on('exit', () => {
                    xinz.sendMessage(from, fs.readFileSync('./media/nulis/images/folio/setelahkanan.jpg'), image, {
                        quoted: msg,
                        caption: `Jangan malas pak...`
                    })
                    limitAdd(sender, limit)
        ...
        //------------------< Text Marker >-------------------
        case prefix + 'retro':
        case prefix + 'retrotext':
        case prefix + 'retroteks':
            if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
        ...
        case prefix + 'cekbapak': // By Ramlan ID
            ...
            xinz.sendMessage(from, cek, text, {
                quoted: msg
            })
            break
            //------------------< Baileys >---------------------
        ...
        case prefix + 'kontag': {
            ...
            if (!q.includes("|")) return reply(`Penggunaan ${command} nomor|nama`)
            if (isNaN(q.split("|")[0])) return reply(`Penggunaan ${command} nomor|nama`)
            xinz.sendContact(from, q.split("|")[0], q.split("|")[1], null, {
                contextInfo: {
                    "mentionedJid": arr
                }
            })
        ...
        case prefix + 'quotes':
            if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            fetchJson('https://rei-api.herokuapp.com/api/quotes')
                .then((kontlo) => {
                    textImg(`_${kontlo.result.quotes}_\n\n―${kontlo.result.author}`)
                    limitAdd(sender, limit)
                })
                .catch((err) => {
                    xinz.sendMess(ownerNumber[0], `${command} Error:` + err)
        ...
        case prefix + 'joke':
        case prefix + 'jokes':
        case prefix + 'randomjoke':
        case prefix + 'randomjokes':
            if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
        ...
        case prefix + 'subredd':
            ...
            break
        case prefix + 'fml':
        case prefix + 'fuckmylife':
        case prefix + 'fmylife':
        case prefix + 'fuvkmylife':
            if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            fetchJson('https://api.zeks.me/api/fml?apikey=' + zekskey)
                .then((kontlo) => {
                    textImg(kontlo.result)
                    limitAdd(sender, limit)
                })
        ...
        case prefix + 'memeindonesia':
        case prefix + 'memesindo':
        case prefix + 'memesindonesia':
            if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            fetchJson('https://rei-api.herokuapp.com/api/fun/memeindo')
                .then((res) => {
                    var qq = `${res.result.link}`
                    xinz.sendFileFromUrl(from, qq, '', msg)
                    limitAdd(sender, limit)
                })
                .catch((err) => {
                    xinz.sendMess(ownerNumber[0], `${command} Error:` + err)
        ...
        case prefix + 'darkjokes':
            if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            fetchJson('https://rei-api.herokuapp.com/api/darkjokes')
                .then((res) => {
                    var qq = `${res.result.link}`
                    xinz.sendFileFromUrl(from, qq, '', msg)
                    limitAdd(sender, limit)
        ...
        case prefix + 'sc':
        case prefix + 'src':
            textImg(`Bot ini menggunakan sc : https://github.com/tahubulat/privbot`)
            break
        case prefix + 'donate':
        case prefix + 'donasi':
        case prefix + 'tos':
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
        ...
        case prefix + 'twitter': {
            ...
            reply(mess.wait)
            fetchJson(`https://rei-api.herokuapp.com/api/dl/twitter?url=${args[1]}`)
                .then((res) => {
                    var pp = res.result
                    const kepsion = `┏┉⌣ ┈̥-̶̯͡..̷̴✽̶┄┈┈┈┈┈┈┈┈┈┈┉┓
┆ *TWITTER DOWNLOAD*
└┈┈┈┈┈┈┈┈┈┈┈⌣ ┈̥-̶̯͡..̷̴✽̶⌣ ✽̶

*Data Berhasil Didapatkan!*
\`\`\`▢ ID : ${pp.id}\`\`\`
\`\`\`▢ Ext : ${pp.media_url[0].content_type}\`\`\`
\`\`\`▢ Retweet : ${pp.retweet_count}\`\`\`
\`\`\`▢ Reply : ${pp.reply_count}\`\`\`
\`\`\`▢ Likes : ${pp.favorite_count}\`\`\`
\`\`\`▢ Quote : ${pp.quote_count}\`\`\`
`
                    xinz.sendFileFromUrl(from, `${pp.media_url[0].url}`, kepsion, msg)
                    limitAdd(sender, limit)
                })
                .catch((err) => {
                    xinz.sendMess(ownerNumber[0], `${command} Error:` + err)
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
        ...
        case prefix + 'igstory':
            ...
            reply(mess.wait)
            fetchJson(`https://rei-api.herokuapp.com/api/dl/igstory?username=${args[1]}`)
                .then((res) => {
                    let list = []
                    let startnum = 1
                    for (let i = 0; i < res.result.length; i++) {
                        let yy = {
                            title: 'Story ke-' + startnum++,
                            rows: [{
        ...
        case prefix + 'tiktok':
            ...
            fetchJson(`https://rei-api.herokuapp.com/api/dl/tiktok?url=${args[1]}`)
                .then((res) => {
                    let qqpp = [{
                        "buttonId": `${prefix}sendfile ${res.result.wm}`,
                        "buttonText": {
                            "displayText": "WITH WM"
                        },
                        "type": "RESPONSE"
                    ...
                    xinz.sendButtons(from, `Hai Kak @${sender.split('@')[0]}`, `TikTok Downloader`, `Mau no wm atau ngga?`, qqpp, false, null, [sender])
                })
                .catch((err) => {
                    xinz.sendMess(ownerNumber[0], `${command} Error:` + err)
                    reply(mess.error.api)
        ...
        case prefix + 'instagram': {
            ...
            reply(mess.wait)
            fetchJson(`https://rei-api.herokuapp.com/api/dl/ig?url=${args[1]}`)
            .then((res) => {
                //var pp = res.result[0]
                let list = []
                let startnum = 1
                ...
                xinz.sendList(from, `List Post`, `List Post yg tersedia di link "${args[1]}" `, `Diurutkan Berdasarkan Tanggal Upload`, `Pilih Disini`, list, msg)
            })
            .catch((err) => {
                xinz.sendMess(ownerNumber[0], `${command} Error:` + err)
                reply(mess.error.api)
...
*Hasil Pencarian : ${q}*\n`
        ...
        case prefix + 'stalkig': {
            ...
            reply(mess.wait)
            fetchJson(`https://rei-api.herokuapp.com/api/search/igstalkz?username=${args[1]}`)
                .then((res) => {
                    //var p = `${res.result}`
...
\`\`\`▢ Followers : ${res.result.follower}\`\`\`
\`\`\`▢ Following : ${res.result.following}\`\`\`
\`\`\`▢ Verified : ${res.result.verified}\`\`\`
\`\`\`▢ Biography :\`\`\` \n${res.result.biography}`
                    xinz.sendFileFromUrl(from, `${res.result.image}`, caption, msg)
                    limitAdd(sender, limit)
                })
                .catch((err) => {
...
\`\`\`▢ Bio :\`\`\`\n${bio}`
        ...
        case prefix + 'tebakgambar': {
            ...
            let anu = await fetchJson(`https://rei-api.herokuapp.com/api/tebakgambar`)
            let anih = anu.result[0].jawaban.toLowerCase()
            game.addgambar(from, anih, gamewaktu, tebakgambar)
            const petunjuk = anu.result[0].jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
            let be = await xinz.prepareMessage(from, await getBuffer(anu.result[0].image), image)
            let qweriio = be.message["ephemeralMessage"] ? be.message.ephemeralMessage : be
            xinz.sendButtons(from, `Tebak Gambar`, monospace(`Silahkan jawab soal berikut ini\n\nWaktu : ${gamewaktu}s`), `Klik dibawah untuk petunjuk`, [{
                "buttonId": `${prefix}hinttt ${petunjuk}`,
                "buttonText": {
                    "displayText": "HINT"
                },
                "type": "RESPONSE"
        ...
        case prefix + 'bc':
            ...
            if (isImage || isQuotedImage) {
                ...
                let media = await xinz.downloadMediaMessage(encmedia)
                for (let i of chiit) {
                    xinz.sendMessage(i.jid, media, image, {
                        caption: q
                    })
                }
                reply(`Sukses`)
                ...
                let media = await xinz.downloadMediaMessage(encmedia)
                for (let i of chiit) {
                    xinz.sendMessage(i.jid, media, video, {
                        caption: q
                    })
                }
                reply(`Sukses`)
        ...
        case prefix + 'odelete':
            ...
            if (isSticker) return xinz.deleteMessage(from, {
                id: msg.message.stickerMessage.contextInfo.stanzaId,
                remoteJid: from,
                fromMe: true
            })
            xinz.deleteMessage(from, {
        ...
        case prefix + 'groupinfo':
            ...
            xinz.sendMessage(from, await getBuffer(pic), image, {
                quoted: msg,
                caption: ingfo,
                contextInfo: {
                ...
                xinz.groupAdd(from, [quotedMsg.sender])
                    .then((res) => {
                        if (res.participants[0][quotedMsg.sender.split("@")[0] + '@c.us'].code === "403") {
                            ...
                            xinz.sendMessage(quotedMsg.sender, {
                                groupName: groupName,
                                groupJid: from,
                ...
                xinz.groupAdd(from, [args[1] + '@s.whatsapp.net'])
                    .then((res) => {
                        if (res.participants[0][args[1] + '@c.us'].code === "403") {
                            ...
                            xinz.sendMessage(args[1] + '@s.whatsapp.net', {
                                groupName: groupName,
                                groupJid: from,
                                inviteCode: au.invite_code,
                                inviteExpiration: au.invite_code_exp,
        ...
        case prefix + 'antibadword':
            ...
            if (args[1] === 'on') {
                ...
                let tahu = [{
                    "buttonId": `${prefix}antibadword on`,
                    "buttonText": {
                        "displayText": "ON"
                    },
                    "type": "RESPONSE"
                ...
                xinz.sendButtons(from, `Anti badword grup`, `Untuk menyalakan dan mematikan fitur antibadword silahkan klik on atau off`, `Untuk tombol yang tidak terlihat, silahkan ketik ${prefix}antibadword on / off`, tahu, false, null, [sender])
            }
            break
        ...
        case prefix + 'mute':
            ...
            if (args[1] === 'on') {
                ...
                xinz.sendButtons(from, `Bot akan di mute di grup ini`, `Pilih On untuk mute bot\nPilih Off untuk unmute bot`, `Untuk tombol yang tidak terlihat, silahkan ketik ${prefix}mute on / off`, tahu, false, null, [sender])
            }
            break
        case prefix + 'antilink':
            ...
            if (args[1] === 'on') {
                ...
                xinz.sendButtons(from, `Anti link grup`, `Untuk menyalakan dan mematikan fitur antilink silahkan klik on atau off`, `Untuk tombol yang tidak terlihat, silahkan ketik ${prefix}antilink on / off`, tahu, false, null, [sender])
            }
            break
        case prefix + 'antiviewonce':
        case prefix + 'antivo':
            if (!isGroup) return reply(mess.OnlyGrup)
            if (!isGroupAdmins && !isOwner) return reply(mess.GrupAdmin)
            //if (args.length === 1) return reply(`Pilih enable atau disable`)
            if (args[1] === 'on') {
                ...
                xinz.sendButtons(from, `Anti view once`, `Untuk menyalakan dan mematikan fitur antivo silahkan klik on atau off`, `Untuk tombol yang tidak terlihat, silahkan ketik ${prefix}antivo on / off`, tahu, false, null, [sender])
            }
            break
        case prefix + 'welcome':
            if (!isGroup) return reply(mess.OnlyGrup)
            ...
            if (args[1] === 'on') {
                ...
                xinz.sendButtons(from, `Anti welcome grup`, `Untuk menyalakan dan mematikan fitur welcome silahkan klik on atau off`, `Untuk tombol yang tidak terlihat, silahkan ketik ${prefix}welcome on / off`, tahu, false, null, [sender])
            }
            break
        ...
        case prefix + 'autostiker':
            ...
            if (args[1] === 'on') {
                ...
                let tahu = [{
                    "buttonId": `${prefix}autostiker on`,
                    "buttonText": {
                        "displayText": "ON"
                    },
                    "type": "RESPONSE"
                }, {
                    "buttonId": `${prefix}autostiker off`,
                    "buttonText": {
                        "displayText": "OFF"
                    },
                    "type": "RESPONSE"
                }]
                xinz.sendButtons(from, `Auto Sticker grup`, `Untuk menyalakan dan mematikan fitur auto sticker silahkan klik on atau off`, `Untuk tombol yang tidak terlihat, silahkan ketik ${prefix}autosticker on / off`, tahu, false, null, [sender])
            }
            break
        case prefix + 'nsfw':
            if (!isGroup) return reply(mess.OnlyGrup)
            ...
            if (args[1] === 'on') {
                ...
                xinz.sendButtons(from, `NSFW`, `Untuk menyalakan dan mematikan fitur nsfw silahkan klik on atau off`, `Untuk tombol yang tidak terlihat, silahkan ketik ${prefix}nsfw on / off`, tahu, false, null, [sender])
            }
            break
        case prefix + 'leveling':
            ...
            if (args[1] === 'on') {
                ...
                xinz.sendButtons(from, `Leveling`, `Untuk menyalakan dan mematikan fitur leveling silahkan klik on atau off`, `Untuk tombol yang tidak terlihat, silahkan ketik ${prefix}leveling on / off`, tahu, false, null, [sender])
            }
            break
        case prefix + 'price':
            if (args[1] === 'pvu') {
                fetchJson(`https://api.pancakeswap.info/api/v2/tokens/0x31471e0791fcdbe82fbf4c44943255e923f1b794`)
                .then ((res) => {
                reply(`\`\`\`Name : ${res.data.name}\`\`\`\n\`\`\`Symbol: ${res.data.symbol}\`\`\`\n\`\`\`Price: $${res.data.price}\`\`\`\n\`\`\`Price BNB: ${res.data.price_BNB}\`\`\``)
            })
            } else if (args[1] === 'petg') {
                fetchJson(`https://api.pancakeswap.info/api/v2/tokens/0x09607078980cbb0665aba9c6d1b84b8ead246aa0`)
                .then ((res) => {
                reply(`\`\`\`Name : ${res.data.name}\`\`\`\n\`\`\`Symbol: ${res.data.symbol}\`\`\`\n\`\`\`Price: $${res.data.price}\`\`\`\n\`\`\`Price BNB: ${res.data.price_BNB}\`\`\``)
            })
            } else {
                let qqpp = [{
                    "buttonId": `${prefix}price pvu`,
                    "buttonText": {
                        "displayText": "PVU"
                    },
                    "type": "RESPONSE"
                }, {
                    "buttonId": `${prefix}price petg`,
                    "buttonText": {
                        "displayText": "PETG"
                    },
                    "type": "RESPONSE"
                }]
                xinz.sendButtons(from, `Hai Kak @${sender.split('@')[0]}`, `Price Token PVU dan PETG`, `Pilih salah 1 bang :v`, qqpp, false, null, [sender])
            }
            break
        default:
            if (isCmd) {
